...
const notificationBaseSchema = z.object({
	...
	guildserverRestart: z.boolean().default(false),
		...
		if (notification) {
			if (notification.notificationType === "slack") {
				form.reset({
					...
					guildserverRestart: notification.guildserverRestart,
	...
	const onSubmit = async (data: NotificationSchema) => {
		...
		if (data.type === "slack") {
			promise = slackMutation.mutateAsync({
				...
				appDeploy: appDeploy,
				guildserverRestart: guildserverRestart,
						...
						<div className="flex flex-col gap-4">
							...
							<div className="grid md:grid-cols-2 gap-4">
								...
								{!isCloud && (
									<FormField
										control={form.control}
										name="guildserverRestart"
										render={({ field }) => (
											<FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm gap-2">
												<div className="space-y-0.5">
													<FormLabel>{BRAND_NAME} Restart</FormLabel>
													<FormDescription>
														Trigger the action when guildserver is restarted.
													</FormDescription>
												</div>
												<FormControl>
													<Switch

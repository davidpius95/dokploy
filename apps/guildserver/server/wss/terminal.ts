...
cat /etc/guildserver/ssh/auto_generated-guildserver-local.pub >> $HOME/.ssh/authorized_keys && \\
echo "✓ ${BRAND_NAME} SSH key added successfully. Reopen the terminal in ${BRAND_NAME} to reconnect."
# ----------------------------------------`;

const COMMAND_TO_GRANT_PERMISSION_ACCESS = `
# ----------------------------------------
sudo chown -R $USER:$USER /etc/guildserver/ssh
# ----------------------------------------
`;

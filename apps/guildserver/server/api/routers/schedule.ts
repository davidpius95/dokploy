...
import { IS_CLOUD, scheduleJob } from "@guildserver/server";
import { removeScheduleJob } from "@guildserver/server";
import { db } from "@guildserver/server/db";
import { deployments } from "@guildserver/server/db/schema/deployment";
import {
	createScheduleSchema,
	schedules,
	updateScheduleSchema,
} from "@guildserver/server/db/schema/schedule";
import { runCommand } from "@guildserver/server/index";
import {
	createSchedule,
	deleteSchedule,
	findScheduleById,
	updateSchedule,
} from "@guildserver/server/services/schedule";
	...
	list: protectedProcedure
		...
		.query(async ({ input }) => {
			const where = {
				...
				"guildserver-server": eq(schedules.userId, input.id),
			};

...
export const notificationRouter = createTRPCRouter({
	...
	testTelegramConnection: adminProcedure
		...
		.mutation(async ({ input }) => {
			try {
				await sendTelegramNotification(input, BRAND_GREETING);
				return true;
			} catch (error) {
				throw new TRPCError({
					code: "BAD_REQUEST",
		...
		.mutation(async ({ input }) => {
			try {
				...
				await sendDiscordNotification(input, {
					...
					description: decorate(">", BRAND_GREETING),
					color: 0xf3f7f4,
				});
	...
	receiveNotification: publicProcedure
		.input(
			z.object({
				ServerType: z.enum(["GuildServer", "Remote"]).default("GuildServer"),
		...
		.mutation(async ({ input }) => {
			try {
				let organizationId = "";
				let ServerName = "";
				if (input.ServerType === "GuildServer") {

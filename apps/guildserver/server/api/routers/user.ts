		...
		.mutation(async ({ input, ctx }) => {
			...
			try {
				await sendEmailNotification(
					...
					`
				<p>You are invited to join ${organization?.name || "organization"} on ${BRAND_NAME}. Click the link to accept the invitation: <a href="${inviteLink}">Accept Invitation</a></p>
					`,
				);
			} catch (error) {
				console.log(error);

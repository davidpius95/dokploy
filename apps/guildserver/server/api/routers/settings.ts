...
import { generateOpenApiDocument } from "@guildserver/trpc-openapi";
...
export const settingsRouter = createTRPCRouter({
	reloadServer: adminProcedure.mutation(async () => {
		...
		await reloadDockerResource("guildserver");
		return true;
	}),
	cleanRedis: adminProcedure.mutation(async () => {
		...
		const { stdout: containerId } = await execAsync(
			`docker ps --filter "name=guildserver-redis" --filter "status=running" -q | head -n 1`,
		);
	...
	reloadRedis: adminProcedure.mutation(async () => {
		...
		await reloadDockerResource("guildserver-redis");
	...
	reloadTraefik: adminProcedure
		...
		.mutation(async ({ input }) => {
			try {
				await reloadDockerResource("guildserver-traefik", input?.serverId);
			} catch (err) {
				console.error(err);
			}
	...
	toggleDashboard: adminProcedure
		...
		.mutation(async ({ input }) => {
			const ports = await readPorts("guildserver-traefik", input.serverId);
	...
	readWebServerTraefikConfig: adminProcedure.query(() => {
		...
		const traefikConfig = readConfig("guildserver");
		return traefikConfig;
	}),
	updateWebServerTraefikConfig: adminProcedure
		...
		.mutation(async ({ input }) => {
			...
			writeConfig("guildserver", input.traefikConfig);
			return true;
		}),
	...
	updateServer: adminProcedure.mutation(async () => {
		...
		// This causes restart of guildserver, thus it will not finish executing properly, so don't await it
	...
	getGuildServerVersion: protectedProcedure.query(() => {
		return packageInfo.version;
	}),
	getReleaseTag: protectedProcedure.query(() => {
		return getGuildServerImageTag();
	}),
	...
	toggleRequests: adminProcedure
		...
		.mutation(async ({ input }) => {
			...
			if (input.enable) {
				const config = {
					accessLog: {
						filePath: "/etc/guildserver/traefik/dynamic/access.log",
						format: "json",
						bufferingSize: 100,
						filters: {
							retryAttempts: true,
	...
	getGuildServerCloudIps: adminProcedure.query(async () => {
		if (!IS_CLOUD) {
			return [];
		}
		const ips = process.env.GUILDSERVER_CLOUD_IPS?.split(",");
		return ips;
	}),
});

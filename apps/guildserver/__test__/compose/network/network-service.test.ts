...
test("It shoudn't add suffix to guildserver-network in services", () => {
	...
	expect(service?.networks).toContain("guildserver-network");
});
...
test("It shoudn't add suffix to guildserver-network in services multiples cases", () => {
	...
	expect(redis?.networks).toHaveProperty("guildserver-network");
	expect(dbNetworks["guildserver-network"]).toBeDefined();
	expect(apiNetworks["guildserver-network"]).toBeDefined();
});

...
import { addSuffixToNetworksRoot, generateRandomHash } from "@guildserver/server";
...
test("It shoudn't add suffix to guildserver-network", () => {
	...
	expect(networks).toBeDefined();
	for (const networkKey of Object.keys(networks)) {
		expect(networkKey).toContain("guildserver-network");
	}
});

...
import type { FileConfig, User } from "@guildserver/server";
import {
...
test("Should read the configuration file", () => {
	...
	expect(config.http?.routers?.["guildserver-router-app"]?.service).toBe(
		"guildserver-service-app",
	);
});
...
test("Should not touch config without host", () => {
	const originalConfig: FileConfig = loadOrCreateConfig("guildserver");

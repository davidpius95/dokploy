...
import { findDestinationById } from "@guildserver/server/services/destination";
...
export const runWebServerBackup = async (backup: BackupSchedule) => {
	...
	try {
		...
		const tempDir = await mkdtemp(join(tmpdir(), `${BRAND_SLUG}-backup-`));
		...
		try {
			...
			if (!containerId) {
				writeStream.write(`${BRAND_NAME} postgres container not found❌\n`);
				writeStream.end();
				throw new Error(`${BRAND_NAME} postgres container not found`);
			}

			writeStream.write(`${BRAND_NAME} postgres container ID: ${containerId}\n`);

			const postgresContainerId = containerId.trim();

			// First dump the database inside the container
			const dumpCommand = `docker exec ${postgresContainerId} pg_dump -v -Fc -U guildserver -d guildserver -f /tmp/database.sql`;

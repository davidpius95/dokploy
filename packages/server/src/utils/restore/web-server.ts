...
import { BRAND_NAME, BRAND_SLUG, IS_CLOUD, paths } from "@guildserver/server/constants";
...
export const restoreWebServerBackup = async (
	...
	try {
		...
		const tempDir = await mkdtemp(join(tmpdir(), `${BRAND_SLUG}-restore-`));
		...
		try {
			...
			const { stdout: postgresContainer } = await execAsync(
				`docker ps --filter "name=guildserver-postgres" --filter "status=running" -q | head -n 1`,
			);

			if (!postgresContainer) {
				throw new Error(`${BRAND_NAME} Postgres container not found`);
			}
			...
			await execAsync(
				`docker exec ${postgresContainerId} psql -U guildserver postgres -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = 'guildserver' AND pid <> pg_backend_pid();"`,
			);

			emit("Dropping existing database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U guildserver postgres -c "DROP DATABASE IF EXISTS guildserver;"`,
			);

			emit("Creating fresh database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U guildserver postgres -c "CREATE DATABASE guildserver;"`,
			);
			...
			await execAsync(
				`docker exec ${postgresContainerId} pg_restore -v -U guildserver -d guildserver /tmp/database.sql`,
			);

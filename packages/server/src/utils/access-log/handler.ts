...
import { findAdmin } from "@guildserver/server/services/admin";
import { updateUser } from "@guildserver/server/services/user";
	...
	try {
		...
		scheduleJob(LOG_CLEANUP_JOB_NAME, cronExpression, async () => {
			try {
				...
				await execAsync("docker exec guildserver-traefik kill -USR1 1");
			} catch (error) {
